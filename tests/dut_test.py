...
def sb_fn(actual_value):
	...
	if not expected_value:
		print("Warning: Received output when no expected value was defined.")
		return
	...
	if actual_value != expected:
		...
		print("-> Success: Output matches expected value.")
...
async def dut_test(dut):
	...
	write_drv = InputDrivers(dut, "", dut.CLK)
	read_drv = OutputDrivers(dut, "", dut.CLK, sb_fn)
	...
	for addr in range(3):
		...
		await read_drv._driver_sent(addr)
	if failed_tests > 0:
		raise Exception(f"Test run failed: {failed_tests} test case(s) did not pass.")
	elif expected_value:
